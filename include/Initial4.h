...
namespace Initial4{
    ...
    void InitialPartitioning(HyperGraph &HyperGraph, FpgaVector &Fpgas,ConstraintChecker &checker) {
        ...
        auto farthestFpga = Initial4::getFarthestFpga(Fpgas);//获得总距离最远的fpga作为初始fpga
        std::vector<int> fpgadis=farthestFpga->distance_neifpga;
        scaleIntegersToMax(fpgadis, maxdis);//将fpga的距离缩放到最大值
        int id = 0;
        for(int dis: fpgadis){
            int distanceToUse = Initial4::findClosestDistance(re_distance, dis);
        ...
        Initial4::growNodes(Fpgas,checker);
    }
}