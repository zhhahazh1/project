...
namespace Initial7{
            ...
            for (size_t i = 0; i < Fpgas.size(); ++i) {
                if (!queues[i].empty()) {
                    ...
                    while (true){
                        ...
                        int randomNumber = rand() % queues[i].size();
                        Node* current = queues[i][randomNumber];
                        queues[i].erase(queues[i].begin() + randomNumber);
    ...
    void InitialPartitioning(HyperGraph &HyperGraph, FpgaVector &Fpgas,ConstraintChecker &checker) {
        ...
        auto farthestFpga = Initial7::getFarthestFpga(Fpgas);//获得总距离最远的fpga作为初始fpga
        std::vector<int> fpgadis=farthestFpga->distance_neifpga;
        scaleIntegersToMax(fpgadis, maxdis);//将fpga的距离缩放到最大值
        int id = 0;
        for(int dis: fpgadis){
            int distanceToUse = Initial7::findClosestDistance(re_distance, dis);
            srand(static_cast<unsigned>(time(0)));
        ...
        Initial7::growNodes(Fpgas,checker);
    }
}