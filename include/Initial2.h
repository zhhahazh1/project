...
namespace Initial2{
    ...
    void growNodes(std::vector<Fpga*>& Fpgas,ConstraintChecker &checker) {
        ...
        while (!allQueuesEmpty) {
            ...
            std::sort(queues.begin(), queues.end(), [](const std::queue<Node*>& a, const std::queue<Node*>& b) {
    ...
    void InitialPartitioning(HyperGraph &HyperGraph, FpgaVector &Fpgas,ConstraintChecker &checker) {
        ...
        Node* startNode=(*maxEdge)->src_node;
        ...
        auto farthestFpga = Initial2::getcenterFpga(Fpgas);//获得总距离最远的fpga作为初始fpga
        std::vector<int> fpgadis=farthestFpga->distance_neifpga;
        scaleIntegersToMax(fpgadis, maxdis);//将fpga的距离缩放到最大值
        int id = 0;
        for(int dis: fpgadis){
            int distanceToUse = Initial2::findClosestDistance(re_distance, dis);
        ...
        Initial2::growNodes(Fpgas,checker);
    }
}