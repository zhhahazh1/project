...
namespace Initial3{
    ...
    void InitialPartitioning(HyperGraph &HyperGraph, FpgaVector &Fpgas,ConstraintChecker &checker) {
        ...
        auto farthestFpga = Initial3::getcenterFpga(Fpgas);//获得总距离最远的fpga作为初始fpga
        std::vector<int> fpgadis=farthestFpga->distance_neifpga;
        scaleIntegersToMax(fpgadis, maxdis);//将fpga的距离缩放到最大值
        int id = 0;
        for(int dis: fpgadis){
            int distanceToUse = Initial3::findClosestDistance(re_distance, dis);
        ...
        Initial3::growNodes(Fpgas,checker);
    }
}